...
if (PRIVATE_KEY_B64) {
  try {
    ...
    if (der.length === 32) {
      ...
      privateKeyObj = crypto.createPrivateKey({ key: Buffer.concat([pkcs8Header, der]), format: 'der', type: 'pkcs8' });
    } else {
      privateKeyObj = crypto.createPrivateKey({ key: der, format: 'der', type: 'pkcs8' });
    }
    console.log('Private key loaded OK:', privateKeyObj.asymmetricKeyType);
  } catch(e) {
...
function buildSignatureHeaders(method, fullPath, authority) {
  ...
  // eBay requires: x-ebay-signature-key, @authority, @method, @path (path only, no query)
  const pathOnly = fullPath.split('?')[0];
  const coveredComponents = '("x-ebay-signature-key" "@authority" "@method" "@path")';
...
async function fetchTransactions(fromDate, toDate) {
  ...
  while (true) {
    ...
    console.log('Finances API status:', resp.status, text.substring(0, 200));
...
app.get('/', (req, res) => res.json({
  ...
  keyLoaded: !!privateKeyObj, keyEnvSet: !!PRIVATE_KEY_B64, jweSet: !!JWE
}));
