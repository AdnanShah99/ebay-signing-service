...
// Convert base64 DER private key to PEM string
function toPem(b64) {
  const der = Buffer.from(b64, 'base64');
  let pkcs8Der;
  if (der.length === 32) {
    // Raw 32-byte Ed25519 key — wrap in PKCS8
    const header = Buffer.from('302e020100300506032b657004220420', 'hex');
    pkcs8Der = Buffer.concat([header, der]);
  } else {
    pkcs8Der = der; // already PKCS8
  }
  const b64pem = pkcs8Der.toString('base64');
  const lines = b64pem.match(/.{1,64}/g).join('\n');
  return `-----BEGIN PRIVATE KEY-----\n${lines}\n-----END PRIVATE KEY-----`;
}

const PRIVATE_KEY_PEM = PRIVATE_KEY_B64 ? toPem(PRIVATE_KEY_B64) : null;
console.log('Private key PEM ready:', !!PRIVATE_KEY_PEM);
...
function buildSignatureHeaders(method, fullPath, authority) {
  if (!PRIVATE_KEY_PEM || !JWE) throw new Error('No signing key configured');

  const created = Math.floor(Date.now() / 1000);
  // eBay documented order: x-ebay-signature-key, @method, @path, @authority
  const coveredComponents = '("x-ebay-signature-key" "@method" "@path" "@authority")';
  const sigInput = `sig1=${coveredComponents};created=${created}`;

  // Path only (no query string) for @path component
  ...
  console.log('Signing base string:\n' + sigBase);

  const privateKey = crypto.createPrivateKey(PRIVATE_KEY_PEM);
  const sigBuffer  = crypto.sign(null, Buffer.from(sigBase, 'utf8'), privateKey);
...
async function fetchTransactions(fromDate, toDate) {
  ...
  while (true) {
    ...
    console.log('API status:', resp.status, text.substring(0, 300));
...
app.get('/', (req, res) => res.json({
  status: 'ok', service: 'eBay Signing Service',
  keyReady: !!PRIVATE_KEY_PEM, jweReady: !!JWE
}));
