...
// Debug: dump ALL transactions (every type) for a date range
app.get('/debug', async (req, res) => {
  ...
  const orderFilter = req.query.order; // optional: show only this order
  try {
    const token = await getAccessToken();
    const authority = 'apiz.ebay.com';
    const fullPath = `/sell/finances/v1/transaction?limit=200`
    ...
    if (orderFilter) txns = txns.filter(t => t.orderId === orderFilter);
    // Summarise the fields we care about
    const summary = txns.map(t => ({
      orderId: t.orderId,
      type: t.transactionType,
      booking: t.bookingEntry,
      amount: t.amount && t.amount.value,
      feeType: t.feeType,
      date: t.transactionDate
    }));
    res.json({ total: data.total, count: txns.length, transactions: summary });
  } catch (err) {
