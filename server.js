...
async function fetchTransactions(fromDate, toDate) {
  ...
  while (true) {
    ...
    console.log('Finances status:', resp.status, text.substring(0, 400));
...
// Debug: dump raw transactions for a date range so we can see the real fields
app.get('/debug', async (req, res) => {
  ...
  try {
    const token = await getAccessToken();
    const authority = 'apiz.ebay.com';
    const fullPath = `/sell/finances/v1/transaction?limit=10`
      + `&filter=transactionType%3A%7BSALE%7D,transactionDate%3A%5B${from}..${to}%5D`;
    const sigHeaders = buildSignatureHeaders('GET', fullPath, authority);
    const resp = await fetch(`https://${authority}${fullPath}`, {
      method: 'GET',
      headers: {
